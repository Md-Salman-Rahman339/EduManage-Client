...
const ClassCard = ({ item, index, enrolledView }) => {
  ...
  const teacherName = teacher?.name || teacher?.email || "Unknown Instructor";
  ...
  return (
    <motion.div variants={fadeIn("up", "spring", index * 0.5, 0.75)}>
      ...
      >
        ...
        <div className="mt-4 flex justify-between items-center text-sm">
          <span className="text-blue-600 font-semibold">${price}</span>
          <Link to={enrolledView ? `/dashboard/myenroll-class/${id}` : `/classDetails/${id}`}>
            <Button>{enrolledView ? "Continue" : "Enroll"}</Button>
          </Link>
          <span className="text-gray-500">Enrolled: {total_enrolment}</span>
        </div>
      </Tilt>
    </motion.div>
  );
